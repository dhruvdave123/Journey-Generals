...
def manageNotes(request):
    ...
    if request.method == "POST":
        ...
        # if form.is_valid():
        #     image=form.cleaned_data['diary_image']
        #     fs=FileSystemStorage()
        #     filename=fs.save(image.name,image)
        #     upload_image_url=fs.url(filename)
        # else:
        #     form=ImageUploadForm()
        try:
...
#         image_formset = DiaryImageFormSet(request.POST, request.FILES)
#         video_formset = DiaryVideoFormSet(request.POST, request.FILES)

#         if diary_form.is_valid() and image_formset.is_valid() and video_formset.is_valid():
#             diary_entry = diary_form.save()
#             images = image_formset.save(commit=False)
#             for image in images:
#                 image.diary = diary_entry
#                 image.save()

#             videos = video_formset.save(commit=False)
#             for video in videos:
#                 video.diary = diary_entry
#                 video.save()

#             return redirect('manageNotes')  # Redirect to a list view or detail view

#     else:
#         diary_form = TravelDiaryForm()
#         image_formset = DiaryImageFormSet()
#         video_formset = DiaryVideoFormSet()
...
#     }
#     return render(request, 'manageNotes.html',context)
